...
\begin{itemize}
    \item Client-side or frontend code: JavaScript code is used to transform the data from the server into a QR image.
    \item WebSocket and server functions: A WebSocket connection is used so that the client can be updated upon confirmation (from the server) of receipt of the correct data from the mobile phone.
    \item Mobile app: We build on top of the \deeid application to add out-of-band form transactions.
    \item Blockchain (deeID): In Section \ref{sec:altdes}, we will see alternative designs and solutions that will utilise the blockchain and the \deeid functionality. We use the messaging function, the proof of identity, and its PKI functionality.
\end{itemize}
