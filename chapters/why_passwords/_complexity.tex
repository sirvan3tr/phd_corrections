\ifthenelse{\boolean{pcppapermadweb}}{
    \input{chapters/why_passwords/__pcp_paper_beginning}
}{}
\vspace*{-0.7cm}
...
\input{chapters/why_passwords/_structure_table}
\ifthenelse{\boolean{pcppapermadweb}}{}{
\input{chapters/why_passwords/_lizardsquad_table}
}
...
Tables~\ref{table:structures} and~\ref{table:ls-structures} use data from labelled passwords and were generated as follows. The datasets were labelled with the respective \textit{Structure} to indicate their structural composition. We then filtered the top 25 most frequent structures and sorted them by dictionary-based permutations. The Hashcat-based attack shows, if we were to perform a dictionary attack and assume they were hashed using the given cryptographic hash functions, how long it would take to crack them using an Nvidia RTX 4090 GPU. Lastly, the \zxcvbn library for Python was used to calculate the average score and number of guesses for each structure in the table. This table and view would not be possible without labelling each password with their respective structure.
...
% 
%\input{chapters/why_passwords/__dataset_complexity}

%\input{chapters/why_passwords/__memorability}
